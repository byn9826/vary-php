...
void vary_array_setValue(zval *this, zval _items)
{
...
zval vary_array_reduce(
  ...
  if (params_num == 1) {
    Bucket *carry;
    if (behavior == 0) {
      carry = Z_ARRVAL_P(_items)->arData;
    } else {
  ...
  for (uint32_t i = start_index; i < items_size; ++i)
  {
    Bucket *carry;
    if (behavior == 0) {
      carry = Z_ARRVAL_P(_items)->arData + i;
    } else {
      carry = Z_ARRVAL_P(_items)->arData + items_size - 1 - i;
    }
    zval args[2];
...
PHP_METHOD(_array, __construct)
{
  ...
  vary_array_setValue(getThis(), _items);
...
PHP_METHOD(OrderedList, remove)
{
  ...
  zend_long target_index = zval_get_long(&carry->val);
  zval_ptr_dtor(&_binarySearch_retval);
  zval return_item = vary_array_removeIndex(vary_array_getValue(getThis()), target_index);
